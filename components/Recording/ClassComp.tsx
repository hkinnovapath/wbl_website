...
const ClassComp: React.FC = () => {
  ...
  // Fetch recordings for the selected batch when both selectedBatch and token change
  useEffect(() => {
    if (selectedBatch && token) {
      ...
      .then(response => {
        setRecordings(response.data.recordings); // Assuming recordings are in response.data.recordings
      })
      .catch(error => {
